...
def test_challenge_response_authentication_is_disabled(host):
    command = """egrep -q '^ChallengeResponseAuthentication no$' /etc/ssh/sshd_config && echo 1 || echo 0"""
    cmd = host.run(command)
    assert '1' in cmd.stdout


def test_challenge_password_authentication_is_disabled(host):
    command = """egrep -q '^PasswordAuthentication no$' /etc/ssh/sshd_config && echo 1 || echo 0"""
    cmd = host.run(command)
    assert '1' in cmd.stdout
